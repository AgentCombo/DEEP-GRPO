...
# LUFFY arm on Qwen2.5-Math-1.5B + MATH.
# Same forest state curriculum as the none/snis arms; the verified teacher
# suffix joins each injected group as an in-batch teacher row trained with
# LUFFY-style policy shaping (f(pi) * A*). Completes the four-arm comparison
# on MATH: sft / none / luffy (this) / snis.
...
# Match the other MATH arms' injection ratio so the four-arm comparison isolates
# the teacher-token loss, not the amount of state-curriculum injection.
...
export RUN_NAME=${RUN_NAME:-Qwen1.5B-MATH-pforest-LUFFY-g${LUFFY_GAMMA}-r${INJECTION_RATIO:-0.125}-${TIMESTAMP}}
