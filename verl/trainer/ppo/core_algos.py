...
def compute_luffy_shaping_loss(
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    gamma: float = 0.1,
) -> tuple[torch.Tensor, torch.Tensor]:
    """LUFFY policy shaping loss for off-policy (teacher) tokens.

    Instead of PPO's importance ratio π_new/π_old with clipping, this uses
    f(π_θ) · advantage where f(x) = x/(x+γ) is a shaping function that
    amplifies learning from low-probability but important off-policy tokens.

    Reference: "Learning to Reason under Off-Policy Guidance" (LUFFY), Sec 3.2, Eq 6.

    Args:
        log_prob: Current policy log probs, shape (batch_size, response_length).
        advantages: Per-token advantages, shape (batch_size, response_length).
        gamma: Shaping function parameter (default 0.1 from LUFFY paper).

    Returns:
        shaping_losses: Per-token loss, same shape as input.
        shaping_weights: f(π_θ) values for monitoring, same shape.
    """
    log_prob_clamped = torch.clamp(log_prob, min=-20.0, max=0.0)
    pi_theta = torch.exp(log_prob_clamped)
    shaping_weights = pi_theta / (pi_theta + gamma)
    shaping_losses = -shaping_weights * advantages
    return shaping_losses, shaping_weights
