...
from verl.utils.fs import copy_to_local, exists as path_exists, is_non_local, local_mkdir_safe
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.extra_optimizers = {
            name: optimizer
            for name, optimizer in (extra_optimizers or {}).items()
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            ...
            if self.should_load_optimizer:
                ...
                for name, optimizer in self.extra_optimizers.items():
                    remote_extra_optim_path = os.path.join(
                        local_path,
                        f"{name}_optim_world_size_{self.world_size}_rank_{self.rank}.pt",
                    )
                    if not path_exists(remote_extra_optim_path):
                        log_with_rank(
                            f"Extra optimizer checkpoint {remote_extra_optim_path} not found; "
                            f"initializing {name} optimizer state from scratch",
                            rank=self.rank,
                            logger=logger,
                            log_only_rank_0=True,
                        )
                        continue
                    local_extra_optim_path = copy_to_local(remote_extra_optim_path)
                    extra_optimizer_state_dict = torch.load(
                        local_extra_optim_path,
                        weights_only=False,
                    )
                    optimizer.load_state_dict(extra_optimizer_state_dict)
                    log_with_rank(
                        f"Loaded {name} optimizer from {remote_extra_optim_path}",
                        rank=self.rank,
                        logger=logger,
                    )
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_optimizer:
                    ...
                    for name, optimizer in self.extra_optimizers.items():
                        extra_optim_path = os.path.join(
                            local_path,
                            f"{name}_optim_world_size_{self.world_size}_rank_{self.rank}.pt",
                        )
                        extra_optimizer_state_dict = optimizer.state_dict()
                        torch.save(extra_optimizer_state_dict, extra_optim_path)
                        log_with_rank(
                            f"Saved {name} optim to {os.path.abspath(extra_optim_path)}",
                            rank=self.rank,
                            logger=logger,
                        )
