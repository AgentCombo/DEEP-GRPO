...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        self.teacher_sft_optimizer = None
        if self._is_actor:
        ...
        if self._is_actor:
            ...
            # Normalize teacher_mini_batch_size: divide by world_size (no rollout.n — teacher has no rollouts)
            dp_size = self.device_mesh.size() // self.ulysses_sequence_parallel_size
            teacher_mbs = self.config.actor.deep_grpo.get("teacher_mini_batch_size", None)
            if teacher_mbs is not None:
                teacher_mbs = int(teacher_mbs)
            if teacher_mbs is not None and dp_size > 1:
                if self.config.actor.deep_grpo.get("teacher_update_mode", "joint") == "post_ppo":
                    assert teacher_mbs % dp_size == 0, (
                        "actor.deep_grpo.teacher_mini_batch_size must be divisible by "
                        f"data parallel size {dp_size}, got {teacher_mbs}"
                    )
                self.config.actor.deep_grpo.teacher_mini_batch_size = teacher_mbs // dp_size
            # micro bsz
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            deep_grpo_config = self.config.actor.get("deep_grpo", {})
            if deep_grpo_config.get("teacher_update_mode", "joint") == "post_ppo":
                teacher_sft_optim_config = deep_grpo_config.get("teacher_sft_optim", {}) or {}

                def teacher_optim_value(name, default):
                    value = teacher_sft_optim_config.get(name, None)
                    return default if value is None else value

                teacher_sft_optimizer = optim.AdamW(
                    actor_module_fsdp.parameters(),
                    lr=teacher_optim_value("lr", optim_config.lr),
                    betas=tuple(
                        teacher_optim_value(
                            "betas",
                            optim_config.get("betas", (0.9, 0.999)),
                        )
                    ),
                    weight_decay=teacher_optim_value(
                        "weight_decay",
                        optim_config.get("weight_decay", 1e-2),
                    ),
                )
                if self.rank == 0:
                    print(
                        "Initialized separate teacher SFT optimizer "
                        f"(lr={teacher_sft_optimizer.param_groups[0]['lr']}, "
                        f"weight_decay={teacher_sft_optimizer.param_groups[0]['weight_decay']})"
                    )
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            if self._is_offload_optimizer:
                ...
                if self.teacher_sft_optimizer is not None:
                    offload_fsdp_optimizer(optimizer=self.teacher_sft_optimizer)
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActor(
                ...
                teacher_sft_optimizer=self.teacher_sft_optimizer,
            )
        ...
        if self._is_actor:
            ...
            self.checkpoint_manager = FSDPCheckpointManager(
                ...
                extra_optimizers={"teacher_sft": self.actor.teacher_sft_optimizer},
            )
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_optimizer:
            ...
            if self.teacher_sft_optimizer is not None:
                load_fsdp_optimizer(optimizer=self.teacher_sft_optimizer, device_id=get_device_id())
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics["actor/lr"] = lr
            metrics["actor/lr_scheduler_step"] = 0
            if int(metrics.get("actor/total_mini_batch_steps", 0)) > 0:
                self.actor_lr_scheduler.step()
                metrics["actor/lr_scheduler_step"] = 1
    ...
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        ...
        if self._is_offload_optimizer:
            ...
            if self.teacher_sft_optimizer is not None:
                offload_fsdp_optimizer(self.teacher_sft_optimizer)
