...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def generate_with_finish_reason(self, prompt_ids: List[int], sampling_params: Dict[str, Any], request_id: str) -> dict:
        ...
        # Support returning per-token logprobs for entropy-based forking (TreeRL)
        want_logprobs = params_dict.pop("return_logprobs", False)
        if want_logprobs:
            params_dict["logprobs"] = 1
        ...
        if want_logprobs and final_res.outputs[0].logprobs is not None:
            token_logprobs = []
            logprobs_list = final_res.outputs[0].logprobs
            for i in range(len(token_ids)):
                if i < len(logprobs_list) and logprobs_list[i] and token_ids[i] in logprobs_list[i]:
                    token_logprobs.append(logprobs_list[i][token_ids[i]].logprob)
                else:
